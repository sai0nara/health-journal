# Documentation Index

> Catalog of documented features. A feature is claimed by listing its three
> document paths here; once claimed, the lint requires all three
> (`Docs/prd/…`, `Docs/psd/…`, `Docs/tests/…`) unless the feature is marked
> `— planned`, which requires only the PRD until the feature is built.
> Cross-references between documents use backticked paths, not wikilinks,
> because the same feature slug exists in three directories.

Last updated: 2026-09-02

## Features

- `Docs/prd/ai-insights.md` — planned
- `Docs/prd/body-measurements.md` — PRD
- `Docs/prd/drive-sync.md` — PRD
- `Docs/prd/entry-logging.md` — PRD
- `Docs/prd/export.md` — PRD
- `Docs/prd/health-connect.md` — PRD
- `Docs/prd/history-feed.md` — PRD
- `Docs/prd/photos-media.md` — PRD
- `Docs/prd/restore-from-backup.md` — PRD
- `Docs/psd/body-measurements.md` — PSD
- `Docs/psd/drive-sync.md` — PSD
- `Docs/psd/entry-logging.md` — PSD
- `Docs/psd/export.md` — PSD
- `Docs/psd/health-connect.md` — PSD
- `Docs/psd/history-feed.md` — PSD
- `Docs/psd/photos-media.md` — PSD
- `Docs/psd/restore-from-backup.md` — PSD
- `Docs/tests/body-measurements.md` — tests
- `Docs/tests/drive-sync.md` — tests
- `Docs/tests/entry-logging.md` — tests
- `Docs/tests/export.md` — tests
- `Docs/tests/health-connect.md` — tests
- `Docs/tests/history-feed.md` — tests
- `Docs/tests/photos-media.md` — tests
- `Docs/tests/restore-from-backup.md` — tests